...
class Admin extends Admin_Controller
{
	...
	public function edit($id = 0)
	{
		...
		$this->_form_data($this->data, true);
		// Build the view using sample/views/admin/form.php
		$this->template->title($this->module_details['name'], lang('swipe:edit'))
		->build('admin/form', $this->data);
	}

	public function _form_data($data, $edit = false)
	{
		...
		if ($edit) {
			for ($i=0; $i < count($files); $i++) {
				$id = $files[$i]->id;
				$template .= '<li><label for="titles[]">'.lang('swipe:title').$files[$i]->name.'</label><div class="input">'.form_input('titles[]', $values->{$id}->title).'</div><label for="links[]">'.lang('swipe:link').$files[$i]->name.'</label><div class="input">'.form_dropdown('links[]', $this->pages, $values->{$id}->link).'</div><div class="input">'.form_input('custom_links[]', $values->{$id}->custom_link, 'placeholder="'.lang('swipe:url').'"').'</div>'.form_hidden('ids[]', $id).'</li>';
			}
		} else {
			for ($i=0; $i < count($files); $i++) {
				$id = $files[$i]->id;
				$template .= '<li><label for="titles[]">'.lang('swipe:title').$files[$i]->name.'</label><div class="input">'.form_input('titles[]', '').'</div><label for="links[]">'.lang('swipe:link').$files[$i]->name.'</label><div class="input">'.form_dropdown('links[]', $this->pages, '').'</div><div class="input">'.form_input('custom_links[]', '', 'placeholder="'.lang('swipe:url').'"').'</div>'.form_hidden('ids[]', $id).'</li>';
			}
		}
